...
echo "============================================================="
echo "============= Welcome to the Moodle CMS installer ==========="
echo "============================================================="
...
echo "============================================================="
echo "======== A robot is now installing Moodle CMS for you ======="
echo "========================= ETC = 120s ========================"
echo "============================================================="
...
if $varnaked;
  then sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/moodle-lemp/main/config.txt' -O /etc/nginx/sites-enabled/"$var01" && sudo sed -i "s/domain/$var01/g" /etc/nginx/sites-enabled/"$var01";
elif $varwww;
  then sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/moodle-lemp/main/config-www.txt' -O /etc/nginx/sites-enabled/"$var01" && sudo sed -i "s/domain/$var011/g" /etc/nginx/sites-enabled/"$var01";
else 
  sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/moodle-lemp/main/config-non-www.txt' -O /etc/nginx/sites-enabled/"$var01" && sudo sed -i "s/domain/$var01/g" /etc/nginx/sites-enabled/"$var01";
fi
...
# installing the app 
cd /tmp
wget https://download.moodle.org/download.php/direct/main311/moodle-latest-311.zip -O latest.zip
unzip -q latest.zip
cd
sudo mv /tmp/moodle /var/www/"$var01"
...
echo "=======================-== DONE! ========================="
echo "=========== Moodle CMS is installed sucessfully =========="
echo "=================== =================== =================="
